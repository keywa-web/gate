...
wscat -c wss://${req.headers.host}/ID
  ...
  async websocketHandler(ws) {
    ...
    ws.on('close', () => {
      ...
      // Cleanup UDP connections untuk WebSocket ini
  ...
  async handleUDPOutbound(targetAddress, targetPort, dataChunk, webSocket, responseHeader, log) {
    return new Promise((resolve) => {
      try {
        ...
        // Create unique key untuk koneksi UDP
        const connectionKey = `${targetAddress}:${targetPort}:${Date.now()}`;
        
        // Buat socket UDP baru
        ...
        log(`New UDP socket created for ${targetAddress}:${targetPort}`);
        
        // Send data
        udpSocket.send(dataChunk, targetPort, targetAddress, (error) => {
          if (error) {
            console.error(`UDP send error to ${targetAddress}:${targetPort}:`, error);
            udpSocket.close();
            this.activeUDPConnections.delete(connectionKey);
            return;
          }
          log(`UDP data sent to ${targetAddress}:${targetPort}`);
        });
        ...
        udpSocket.on('message', (message, rinfo) => {
          if (webSocket.readyState === WebSocket.OPEN) {
            ...
            log(`Received UDP from ${rinfo.address}:${rinfo.port}`);
          }
        });
        ...
        // Handle close
        udpSocket.on('close', () => {
          log(`UDP socket closed for ${targetAddress}:${targetPort}`);
          this.activeUDPConnections.delete(connectionKey);
        });
        
        // Auto-close setelah 30 detik idle
        ...
        console.error(`Error in UDP handler: ${e.message}`);
      }
    });
  }
  ...
  start(port = process.env.PORT || 3000) {
    ...
    server.listen(port, '0.0.0.0', () => {
      ...
      console.log(`🚀 UDP Native: Enabled`);
